...
🟢 <span style="color:rgb(86, 203, 195)">**Сервис поднят прямо сейчас**</span> и доступен для каждого желающего по адресу в описании github. Редирект на Google: http://t.tar.nz/OOsZK. Методы работы с сервисом описаны далее в документации. Для self-hosted решения достаточно **создать .env** файл на основе .env.dev и запустить **docker-compose**.
...
1. Пользователю не нужно проходить отдельную регистрацию для создания ссылок, авторизация существует на основе токена, который выдается при создании ссылки (если в ней не указывается существующий token), с помощью которого он сможет создавать ссылки и управлять ими.
...
* Также проект проходит нагрузочное тестирование с помощью locust в 50 запросов в секунду.
* Настроен GitAction, который валидирует каждый коммит и PR - тестами, которые покрывают более 96% логики сервиса. Таблица % покрытия функций тестами находится в [GitAction](https://github.com/onleep/turl/actions) -> build -> test with pytest.
* Запуск тестов в проекте происходит командой `pytest`, дополнительные параметры указывать не нужно - в проекте находится конфиг `pytest.ini`, который настраивает pytest. Нагрузочное тестирование происходит с помощью команд `export PYTHONPATH=app` && `locust --config locust.conf`
